...
def generate_candidates(
    ...
    multi_temperature=True时，使用多个温度生成更多样化的候选。

    Args:
        model: 语言模型
        tokenizer: 分词器
        prompts: 输入prompt列表
        num_candidates: 每个prompt生成的候选数（per temperature）
        temperature: 采样温度（单温度模式）或基准温度（多温度模式）
        top_p: nucleus sampling参数
        max_new_tokens: 最大生成长度（摘要任务128足够）
        batch_size: 每批处理的prompt数
        multi_temperature: 是否使用多温度生成

    Returns:
        候选输出列表，shape [num_prompts, total_candidates]
    """
    model.eval()
    ...
    if multi_temperature:
        temperatures = [0.5, 0.8, 1.2, 1.8]
    else:
        temperatures = [temperature]

    all_candidates = [[] for _ in range(len(prompts))]
    total = len(prompts)
        ...
        for i in range(0, total, batch_size):
            ...
            for j in range(batch_size_actual):
                for k in range(num_candidates):
                    ...
                    all_candidates[i + j].append(response)

            if (i + batch_size) % 100 == 0 or i + batch_size >= total:
                print(f"    {min(i + batch_size, total)}/{total} prompts")
